      ...
      {selected && profiles && (
        <div style={styles.profileBox}>
          ...
          <p style={{ color: '#888', fontSize: '13px', marginBottom: '10px' }}>
            Live profile discovery requires the deep search service, which is not currently deployed.
          </p>
          {['linkedin', 'github', 'twitter'].map(platform => (
            <div key={platform} style={styles.profileCard}>
              <strong style={styles.platformName}>{platform.toUpperCase()}</strong>
              {profiles[platform] && profiles[platform].found ? (
                <div>
